...
static ID id_complex, id_rational, id_ivar_compiler, id_ivar_root;
...
static VALUE compile_string(VALUE node, VALUE schema) {
  ...
  r->as.string.decode_base64 = RB_TYPE_P(encoding, T_STRING) && rb_str_equal(encoding, STATIC_STRING(BASE64)) ? Qtrue : Qfalse;
  r->as.string.parse_json = RB_TYPE_P(media_type, T_STRING) && rb_str_equal(media_type, STATIC_STRING(APPLICATION_JSON)) ? Qtrue : Qfalse;
...
static VALUE compile_array(VALUE self, VALUE node, VALUE parent, VALUE schema) {
  ...
  r->as.array.unique = hget(schema, STATIC_STRING(UNIQUE_ITEMS));
  ...
  if (RB_TYPE_P(items, T_ARRAY)) { r->as.array.items = compile_program_list(self, node, parent, items, STATIC_STRING(ITEMS)); r->as.array.items_list = Qtrue; }
  ...
  r->as.array.count_contains = rb_hash_lookup2(keywords, STATIC_STRING(MIN_CONTAINS), Qundef) == Qundef ? Qfalse : Qtrue;
...
static VALUE evaluator_initialize(int argc,VALUE*argv,VALUE self){VALUE graph,compiler,root,options;rb_scan_args(argc,argv,"3:",&graph,&compiler,&root,&options);evaluator_t*e;TypedData_Get_Struct(self,evaluator_t,&evaluator_type,e);RB_OBJ_WRITE(self,&e->graph,graph);RB_OBJ_WRITE(self,&e->compiler,compiler);RB_OBJ_WRITE(self,&e->root,root);rb_ivar_set(self,id_ivar_compiler,compiler);rb_ivar_set(self,id_ivar_root,root);RB_OBJ_WRITE(self,&e->regexps,rb_hash_new());RB_OBJ_WRITE(self,&e->resolved,rb_hash_new());rb_funcall(e->resolved,id_compare_by_identity,0);RB_OBJ_WRITE(self,&e->active,rb_hash_new());rb_funcall(e->active,id_compare_by_identity,0);RB_OBJ_WRITE(self,&e->dynamic_scope,rb_ary_new());e->content=RTEST(rb_hash_aref(options,sym_content));e->format=RTEST(rb_hash_aref(options,sym_format));return self;}
...
static void check_number_detail(evaluator_t*e,rule_t*r,VALUE value){VALUE actual=(r->mask&NUM_MULTIPLE_OF)?decimal(value):value;VALUE keys[]={STATIC_STRING(MAXIMUM),STATIC_STRING(MINIMUM),STATIC_STRING(EXCLUSIVE_MAXIMUM),STATIC_STRING(EXCLUSIVE_MINIMUM)};for(int i=0;i<4;i++)if(r->mask&(1u<<i)){int comparison=compare_values(actual,r->as.slots[i]);bool invalid=(i==0&&comparison>0)||(i==1&&comparison<0)||(i==2&&comparison>=0)||(i==3&&comparison<=0);if(invalid){VALUE a[]={keys[i],r->as.slots[i]};add_error(e,keys[i],error_message(id_error_numeric_limit,2,a),true);}}if(r->mask&NUM_MULTIPLE_OF){VALUE divisor=decimal(r->as.slots[4]);bool ok=RTEST(rb_funcall(divisor,id_positive_p,0))&&RTEST(rb_funcall(rb_funcall(decimal(value),id_remainder,1,divisor),id_zero_p,0));if(!ok){VALUE a[]={r->as.slots[4]};add_error(e,STATIC_STRING(MULTIPLE_OF),error_message(id_error_multiple_of,1,a),true);}}}
...
static void check_string_detail(evaluator_t*e,rule_t*r,VALUE value){long len=NUM2LONG(rb_funcall(value,id_length,0));if(!NIL_P(r->as.slots[0])&&len>NUM2LONG(r->as.slots[0])){VALUE a[]={STATIC_STRING(MAX_LENGTH),r->as.slots[0],LONG2NUM(len)};add_error(e,STATIC_STRING(MAX_LENGTH),error_message(id_error_size,3,a),true);}if(!NIL_P(r->as.slots[1])&&len<NUM2LONG(r->as.slots[1])){VALUE a[]={STATIC_STRING(MIN_LENGTH),r->as.slots[1],LONG2NUM(len)};add_error(e,STATIC_STRING(MIN_LENGTH),error_message(id_error_size,3,a),true);}if(!NIL_P(r->as.slots[2])){int state=0;VALUE regexp=rb_protect(protected_func,(VALUE)&(struct protected_call){mNativeSupport,id_regexp,1,{r->as.slots[2]}},&state);if(state){rb_set_errinfo(Qnil);VALUE a[]={r->as.slots[2]};add_error(e,STATIC_STRING(PATTERN),error_message(id_error_invalid_pattern,1,a),true);}else if(!RTEST(rb_funcall(regexp,id_match_p,1,value))){VALUE a[]={r->as.slots[2]};add_error(e,STATIC_STRING(PATTERN),error_message(id_error_pattern,1,a),true);}}if(!NIL_P(r->as.slots[3])&&(e->format||RTEST(r->as.slots[4]))&&!RTEST(rb_funcall(r->as.slots[3],id_call,1,value))){VALUE a[]={rb_funcall(r->as.slots[3],id_name,0)};add_error(e,STATIC_STRING(FORMAT),error_message(id_error_format,1,a),true);}if(e->content&&(RTEST(r->as.slots[5])||RTEST(r->as.slots[6]))&&!RTEST(rb_funcall(mNativeSupport,id_valid_content_p,3,value,r->as.slots[5],r->as.slots[6]))){if(RTEST(r->as.slots[5]))add_message0(e,STATIC_STRING(CONTENT_ENCODING),id_error_content_encoding);else add_message0(e,STATIC_STRING(CONTENT_MEDIA_TYPE),id_error_content_media_type);}}

static evaluation_t check_array_detail(evaluator_t*e,rule_t*r,VALUE value,evaluation_t prior){evaluation_t out=evaluation(true);long before=e->error_count,len=RARRAY_LEN(value);if(!NIL_P(r->as.slots[0])&&len>NUM2LONG(r->as.slots[0])){VALUE a[]={STATIC_STRING(MAX_ITEMS),r->as.slots[0],LONG2NUM(len)};add_error(e,STATIC_STRING(MAX_ITEMS),error_message(id_error_size,3,a),true);}if(!NIL_P(r->as.slots[1])&&len<NUM2LONG(r->as.slots[1])){VALUE a[]={STATIC_STRING(MIN_ITEMS),r->as.slots[1],LONG2NUM(len)};add_error(e,STATIC_STRING(MIN_ITEMS),error_message(id_error_size,3,a),true);}if(RTEST(r->as.slots[2])){bool duplicate=false;for(long i=1;i<len&&!duplicate;i++)for(long j=0;j<i;j++)if(json_equal(rb_ary_entry(value,j),rb_ary_entry(value,i))){duplicate=true;break;}if(duplicate)add_message0(e,STATIC_STRING(UNIQUE_ITEMS),id_error_unique_items);}
  if(!NIL_P(r->as.slots[3]))for(long i=0;i<RARRAY_LEN(r->as.slots[3])&&i<len;i++){detail_at(e,rb_ary_entry(r->as.slots[3],i),rb_ary_entry(value,i),LONG2NUM(i),true,STATIC_STRING(PREFIX_ITEMS),LONG2NUM(i),true);add_unique(&out.items,LONG2NUM(i));}
  ...
  else if(!NIL_P(r->as.slots[4])){long start=NIL_P(r->as.slots[3])?0:RARRAY_LEN(r->as.slots[3]);for(long i=start;i<len;i++){detail_at(e,r->as.slots[4],rb_ary_entry(value,i),LONG2NUM(i),true,STATIC_STRING(ITEMS),Qnil,false);add_unique(&out.items,LONG2NUM(i));}}
  if(!NIL_P(r->as.slots[7])){VALUE matched=rb_ary_new();for(long i=0;i<len;i++)if(evaluate_program(e,r->as.slots[7],rb_ary_entry(value,i)).valid)rb_ary_push(matched,LONG2NUM(i));long count=RARRAY_LEN(matched);if(count<NUM2LONG(r->as.slots[8])||compare_values(LONG2NUM(count),r->as.slots[9])>0){VALUE a[]={LONG2NUM(count),r->as.slots[8],r->as.slots[9]};add_error(e,STATIC_STRING(CONTAINS),error_message(id_error_contains,3,a),true);}merge_locations(&out.items,matched);}
  if(!NIL_P(r->as.slots[11]))for(long i=0;i<len;i++){VALUE index=LONG2NUM(i);if(has_location(prior.items,index)||has_location(out.items,index))continue;detail_at(e,r->as.slots[11],rb_ary_entry(value,i),index,true,STATIC_STRING(UNEVALUATED_ITEMS),Qnil,false);add_unique(&out.items,index);}
  out.valid=e->error_count==before;return out;}

static evaluation_t check_object_detail(evaluator_t*e,rule_t*r,VALUE value,evaluation_t prior){evaluation_t out=evaluation(true);long before=e->error_count,len=RHASH_SIZE(value);if(!NIL_P(r->as.slots[0])&&len>NUM2LONG(r->as.slots[0])){VALUE a[]={STATIC_STRING(MAX_PROPERTIES),r->as.slots[0],LONG2NUM(len)};add_error(e,STATIC_STRING(MAX_PROPERTIES),error_message(id_error_size,3,a),true);}if(!NIL_P(r->as.slots[1])&&len<NUM2LONG(r->as.slots[1])){VALUE a[]={STATIC_STRING(MIN_PROPERTIES),r->as.slots[1],LONG2NUM(len)};add_error(e,STATIC_STRING(MIN_PROPERTIES),error_message(id_error_size,3,a),true);}if(!NIL_P(r->as.slots[2]))for(long i=0;i<RARRAY_LEN(r->as.slots[2]);i++){VALUE name=rb_ary_entry(r->as.slots[2],i);if(!RTEST(rb_funcall(value,id_key_p,1,name))){VALUE a[]={name};add_error(e,STATIC_STRING(REQUIRED),error_message(id_error_required,1,a),true);}}
  VALUE keys=rb_funcall(value,id_keys,0);for(long i=0;i<RARRAY_LEN(keys);i++){VALUE name=rb_ary_entry(keys,i),item=rb_hash_aref(value,name);bool matched=false;VALUE child=rb_hash_lookup2(r->as.slots[3],name,Qundef);if(child!=Qundef){matched=true;detail_at(e,child,item,name,true,STATIC_STRING(PROPERTIES),name,true);add_unique(&out.properties,name);}if(!NIL_P(r->as.slots[4])){VALUE patterns=rb_funcall(r->as.slots[4],id_keys,0);for(long j=0;j<RARRAY_LEN(patterns);j++){VALUE pattern=rb_ary_entry(patterns,j);if(RTEST(rb_funcall(regexp_for(e,pattern),id_match_p,1,name))){matched=true;detail_at(e,rb_hash_aref(r->as.slots[4],pattern),item,name,true,STATIC_STRING(PATTERN_PROPERTIES),pattern,true);add_unique(&out.properties,name);}}}if(!matched&&!NIL_P(r->as.slots[5])){detail_at(e,r->as.slots[5],item,name,true,STATIC_STRING(ADDITIONAL_PROPERTIES),Qnil,false);add_unique(&out.properties,name);}}
  if(!NIL_P(r->as.slots[6]))for(long i=0;i<RARRAY_LEN(keys);i++){VALUE name=rb_ary_entry(keys,i);detail_at(e,r->as.slots[6],name,name,true,STATIC_STRING(PROPERTY_NAMES),Qnil,false);}
  if(!NIL_P(r->as.slots[7])){VALUE names=rb_funcall(r->as.slots[7],id_keys,0);for(long i=0;i<RARRAY_LEN(names);i++){VALUE name=rb_ary_entry(names,i);if(!RTEST(rb_funcall(value,id_key_p,1,name)))continue;VALUE dep=rb_hash_aref(r->as.slots[7],name);if(RB_TYPE_P(dep,T_ARRAY)){for(long j=0;j<RARRAY_LEN(dep);j++){VALUE req=rb_ary_entry(dep,j);if(!RTEST(rb_funcall(value,id_key_p,1,req))){VALUE a[]={name,req};add_error(e,STATIC_STRING(DEPENDENCIES),error_message(id_error_dependent_required,2,a),true);}}}else{evaluation_t x=detail_at(e,dep,value,Qnil,false,STATIC_STRING(DEPENDENCIES),name,true);if(x.valid)merge_locations(&out.properties,x.properties);}}}
  if(!NIL_P(r->as.slots[8])){VALUE names=rb_funcall(r->as.slots[8],id_keys,0);for(long i=0;i<RARRAY_LEN(names);i++){VALUE name=rb_ary_entry(names,i);if(!RTEST(rb_funcall(value,id_key_p,1,name)))continue;VALUE reqs=rb_hash_aref(r->as.slots[8],name);for(long j=0;j<RARRAY_LEN(reqs);j++){VALUE req=rb_ary_entry(reqs,j);if(!RTEST(rb_funcall(value,id_key_p,1,req))){VALUE a[]={name,req};add_error(e,STATIC_STRING(DEPENDENT_REQUIRED),error_message(id_error_dependent_required,2,a),true);}}}}
  if(!NIL_P(r->as.slots[9])){VALUE names=rb_funcall(r->as.slots[9],id_keys,0);for(long i=0;i<RARRAY_LEN(names);i++){VALUE name=rb_ary_entry(names,i);if(!RTEST(rb_funcall(value,id_key_p,1,name)))continue;evaluation_t x=detail_at(e,rb_hash_aref(r->as.slots[9],name),value,Qnil,false,STATIC_STRING(DEPENDENT_SCHEMAS),name,true);if(x.valid)merge_locations(&out.properties,x.properties);}}
  if(!NIL_P(r->as.slots[10]))for(long i=0;i<RARRAY_LEN(keys);i++){VALUE name=rb_ary_entry(keys,i);if(has_location(prior.properties,name)||has_location(out.properties,name))continue;detail_at(e,r->as.slots[10],rb_hash_aref(value,name),name,true,STATIC_STRING(UNEVALUATED_PROPERTIES),Qnil,false);add_unique(&out.properties,name);}
  out.valid=e->error_count==before;return out;}

static evaluation_t check_combiner_detail(evaluator_t*e,uint8_t op,VALUE operand,VALUE value){evaluation_t out=evaluation(true);if(op==OP_ALL_OF){for(long i=0;i<RARRAY_LEN(operand);i++)merge_evaluation(&out,detail_at(e,rb_ary_entry(operand,i),value,Qnil,false,STATIC_STRING(ALL_OF),LONG2NUM(i),true));}else if(op==OP_ANY_OF||op==OP_ONE_OF){long matches=0;evaluation_t matched=evaluation(true);for(long i=0;i<RARRAY_LEN(operand);i++){evaluation_t x=evaluate_program(e,rb_ary_entry(operand,i),value);if(x.valid){matches++;merge_evaluation(&matched,x);}}if(op==OP_ANY_OF&&matches==0)add_message0(e,STATIC_STRING(ANY_OF),id_error_any_of);else if(op==OP_ONE_OF&&matches!=1){VALUE a[]={LONG2NUM(matches)};add_error(e,STATIC_STRING(ONE_OF),error_message(id_error_one_of,1,a),true);}else merge_evaluation(&out,matched);}else if(op==OP_NOT){if(evaluate_program(e,operand,value).valid)add_message0(e,STATIC_STRING(NOT),id_error_not);}else{rule_t*r;TypedData_Get_Struct(operand,rule_t,&rule_type,r);evaluation_t condition=evaluate_program(e,r->as.slots[0],value);if(condition.valid){merge_evaluation(&out,condition);if(!NIL_P(r->as.slots[1]))merge_evaluation(&out,detail_at(e,r->as.slots[1],value,Qnil,false,STATIC_STRING(THEN),Qnil,false));}else if(!NIL_P(r->as.slots[2]))merge_evaluation(&out,detail_at(e,r->as.slots[2],value,Qnil,false,STATIC_STRING(ELSE),Qnil,false));}return out;}
...
void Init_schemurai_native(void) {
  ...
  id_nodes=rb_intern_const("nodes"); id_resolve=rb_intern_const("resolve"); id_dynamic_anchor=rb_intern_const("dynamic_anchor"); id_root=rb_intern_const("root"); id_ref_siblings_p=rb_intern_const("ref_siblings?"); id_format_assertion_p=rb_intern_const("format_assertion?"); id_keywords=rb_intern_const("keywords"); id_key_p=rb_intern_const("key?"); id_call=rb_intern_const("call"); id_finite_p=rb_intern_const("finite?"); id_to_i=rb_intern_const("to_i"); id_to_s=rb_intern_const("to_s"); id_remainder=rb_intern_const("remainder"); id_zero_p=rb_intern_const("zero?"); id_positive_p=rb_intern_const("positive?"); id_name=rb_intern_const("name"); id_regexp=rb_intern_const("regexp"); id_valid_content_p=rb_intern_const("valid_content?"); id_new=rb_intern_const("new"); id_compile=rb_intern_const("compile"); id_valid_p=rb_intern_const("valid?"); id_evaluated_properties=rb_intern_const("evaluated_properties"); id_evaluated_items=rb_intern_const("evaluated_items");
  id_keys=rb_intern_const("keys"); id_include_p=rb_intern_const("include?"); id_compare_by_identity=rb_intern_const("compare_by_identity"); id_compare=rb_intern_const("<=>"); id_length=rb_intern_const("length"); id_match_p=rb_intern_const("match?"); id_gsub=rb_intern_const("gsub"); id_message=rb_intern_const("message"); id_validate=rb_intern_const("validate"); id_complex=rb_intern_const("Complex"); id_rational=rb_intern_const("Rational"); id_ivar_compiler=rb_intern_const("@compiler"); id_ivar_root=rb_intern_const("@root");
  ...
  rb_funcall(mVM,rb_intern_const("private_constant"),1,ID2SYM(rb_intern_const("Rule")));
