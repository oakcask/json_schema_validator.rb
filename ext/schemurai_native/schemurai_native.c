...
static evaluation_t valid_array(evaluator_t*e,rule_t*r,VALUE value,evaluation_t prior){
  ...
  if(!NIL_P(r->as.array.prefix_items))for(long i=0;i<RARRAY_LEN(r->as.array.prefix_items)&&i<len;i++){evaluation_t x=evaluate_program(e,rb_ary_entry(r->as.array.prefix_items,i),rb_ary_entry(value,i));if(!x.valid)return evaluation(false);add_unique(&out.items,LONG2NUM(i));}
  ...
  else if(!NIL_P(r->as.array.items)){long start=NIL_P(r->as.array.prefix_items)?0:RARRAY_LEN(r->as.array.prefix_items);for(long i=start;i<len;i++){evaluation_t x=evaluate_program(e,r->as.array.items,rb_ary_entry(value,i));if(!x.valid)return evaluation(false);add_unique(&out.items,LONG2NUM(i));}}
  if(!NIL_P(r->as.array.contains)){long matches=0;VALUE matched=rb_ary_new();for(long i=0;i<len;i++)if(evaluate_program(e,r->as.array.contains,rb_ary_entry(value,i)).valid){matches++;rb_ary_push(matched,LONG2NUM(i));}if(matches<NUM2LONG(r->as.array.min_contains)||compare_values(LONG2NUM(matches),r->as.array.max_contains)>0)return evaluation(false);merge_locations(&out.items,matched);}
...
static evaluation_t valid_object(evaluator_t*e,rule_t*r,VALUE value,evaluation_t prior){
  ...
  for(long i=0;i<RARRAY_LEN(keys);i++){VALUE name=rb_ary_entry(keys,i),item=rb_hash_aref(value,name);bool matched=false;VALUE child=rb_hash_lookup2(r->as.object.properties,name,Qundef);if(child!=Qundef){matched=true;if(!evaluate_program(e,child,item).valid)return evaluation(false);add_unique(&out.properties,name);}if(!NIL_P(r->as.object.patterns)){VALUE patterns=rb_funcall(r->as.object.patterns,id_keys,0);for(long j=0;j<RARRAY_LEN(patterns);j++){VALUE pattern=rb_ary_entry(patterns,j);struct protected_call c={regexp_for(e,pattern),id_match_p,1,{name}};if(protected_truth(&c)){matched=true;if(!evaluate_program(e,rb_hash_aref(r->as.object.patterns,pattern),item).valid)return evaluation(false);add_unique(&out.properties,name);}}}if(!matched&&!NIL_P(r->as.object.additional)){if(!evaluate_program(e,r->as.object.additional,item).valid)return evaluation(false);add_unique(&out.properties,name);}}
  ...
  if(!NIL_P(r->as.object.dependent_schemas)){VALUE names=rb_funcall(r->as.object.dependent_schemas,id_keys,0);for(long i=0;i<RARRAY_LEN(names);i++){VALUE name=rb_ary_entry(names,i);if(!RTEST(rb_funcall(value,id_key_p,1,name)))continue;evaluation_t x=evaluate_program(e,rb_hash_aref(r->as.object.dependent_schemas,name),value);if(!x.valid)return evaluation(false);merge_locations(&out.properties,x.properties);}}
