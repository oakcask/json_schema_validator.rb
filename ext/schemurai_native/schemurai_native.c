...
static const rb_data_type_t program_type = {"Schemurai::VM::Program",
...
static const rb_data_type_t rule_type = {"Schemurai::VM::Rule",
...
static const rb_data_type_t compiler_type = {"Schemurai::VM::Compiler",
...
static const rb_data_type_t evaluator_type = {"Schemurai::VM::Evaluator",
...
static VALUE compiler_alloc(VALUE klass) {
  compiler_t *c;
  return TypedData_Make_Struct(klass, compiler_t, &compiler_type, c);
}
static VALUE evaluator_alloc(VALUE klass) {
  evaluator_t *e;
...
static VALUE hget(VALUE hash, VALUE key) {
  return rb_hash_aref(hash, key);
}
static bool hkey(VALUE hash, VALUE key) {
...
static evaluation_t evaluation(bool valid) {
  evaluation_t r = {valid, Qnil, Qnil};
  return r;
}
...
static VALUE protected_func(VALUE arg) {
...
static void active_leave(evaluator_t *e, VALUE source, VALUE instance) {
  e->active_length--;
}
...
static VALUE safe_target(evaluator_t *e, VALUE source, VALUE rule, uint8_t opcode, int *state) {
...
static VALUE evaluator_backend(VALUE self) {
  return sym_vm;
}
static VALUE ruby_evaluator(evaluator_t *e) {
  program_t *p;
...
static VALUE evaluator_valid(VALUE self, VALUE instance) {
  evaluator_t *e;
...
static VALUE evaluator_validate(VALUE self, VALUE instance) {
  evaluator_t *e;
  TypedData_Get_Struct(self, evaluator_t, &evaluator_type, e);
