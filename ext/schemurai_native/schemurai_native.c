...
typedef struct {
  ...
  union {
    ...
    struct { VALUE maximum, minimum, exclusive_maximum, exclusive_minimum, multiple_of, multiple_decimal; } number;
    struct {
...
typedef struct { VALUE source, instance; } active_entry_t;
#define CACHED_SCOPE_LIMIT 8
typedef struct { VALUE rule,target,resources[CACHED_SCOPE_LIMIT]; uint8_t length; } scope_cache_entry_t;
typedef struct {
...
#define VISIT(field) do { if (compact) { COMPACT_RULE_VALUE(field); } else { MARK_RULE_VALUE(field); } } while (0)
  switch (r->kind) {
    ...
    case RULE_NUMBER: VISIT(number.maximum); VISIT(number.minimum); VISIT(number.exclusive_maximum); VISIT(number.exclusive_minimum); VISIT(number.multiple_of); VISIT(number.multiple_decimal); break;
    ...
    case RULE_OBJECT:
      VISIT(object.max_properties); VISIT(object.min_properties); VISIT(object.required); VISIT(object.properties); VISIT(object.patterns); VISIT(object.pattern_names);
...
static void evaluator_mark(void *ptr) {
  ...
  for(size_t i=0;i<e->scope_cache_length;i++){rb_gc_mark_movable(e->scope_cache[i].rule);rb_gc_mark_movable(e->scope_cache[i].target);for(uint8_t j=0;j<e->scope_cache[i].length;j++)rb_gc_mark_movable(e->scope_cache[i].resources[j]);}
}
static void evaluator_compact(void *ptr) {
  ...
  for(size_t i=0;i<e->scope_cache_length;i++){e->scope_cache[i].rule=rb_gc_location(e->scope_cache[i].rule);e->scope_cache[i].target=rb_gc_location(e->scope_cache[i].target);for(uint8_t j=0;j<e->scope_cache[i].length;j++)e->scope_cache[i].resources[j]=rb_gc_location(e->scope_cache[i].resources[j]);}
}
static void evaluator_free(void *ptr) { evaluator_t *e = ptr; xfree(e->active);xfree(e->scope_cache); xfree(e); }
static size_t evaluator_size(const void *ptr) { const evaluator_t *e = ptr; return sizeof(*e) + e->active_capacity * sizeof(active_entry_t)+e->scope_cache_capacity*sizeof(scope_cache_entry_t); }
static const rb_data_type_t evaluator_type = {
  "Schemurai::VM::Evaluator", {evaluator_mark, evaluator_free, evaluator_size, evaluator_compact},
...
static bool hkey(VALUE hash, VALUE key) { return rb_hash_lookup2(hash, key, Qundef) != Qundef; }
...
static VALUE compile_number(VALUE schema) {
  ...
  if(r->mask&NUM_MULTIPLE_OF)RB_OBJ_WRITE(obj,&r->as.number.multiple_decimal,decimal(r->as.number.multiple_of));
...
static VALUE compile_object(VALUE self, VALUE node, VALUE parent, VALUE schema) {
  ...
  VALUE map = compile_map(self, node, parent, schema, STATIC_STRING(PATTERN_PROPERTIES)); r->as.object.patterns = RHASH_EMPTY_P(map) ? Qnil : map;r->as.object.pattern_names=NIL_P(r->as.object.patterns)?Qnil:rb_obj_freeze(rb_funcall(r->as.object.patterns,id_keys,0));
  ...
  VALUE *values[] = {&r->as.object.max_properties, &r->as.object.min_properties, &r->as.object.required, &r->as.object.properties, &r->as.object.patterns, &r->as.object.pattern_names, &r->as.object.additional, &r->as.object.property_names, &r->as.object.dependencies, &r->as.object.dependent_required, &r->as.object.dependent_schemas, &r->as.object.unevaluated};
  for (int i = 0; i < 12; i++) RB_OBJ_WRITTEN(obj, Qundef, *values[i]);
  return rb_obj_freeze(obj);
}
...
static VALUE compiler_compile(VALUE self, VALUE node) {
  ...
  RB_OBJ_WRITE(program, &p->node, node); p->resource = Qnil; p->dynamic_anchor = Qnil;
...
finish:
  if(p->flags&FLAG_DYNAMIC_SCOPE)RB_OBJ_WRITE(program,&p->resource,rb_funcall(node,id_resource,0));
...
static void add_unique(VALUE *list, VALUE value) {
  if (NIL_P(*list)) { *list=value; return; }
  if (RB_TYPE_P(*list,T_ARRAY)) { if (!RTEST(rb_ary_includes(*list,value))) rb_ary_push(*list,value); return; }
  if (!RTEST(rb_equal(*list,value))) *list=rb_ary_new_from_args(2,*list,value);
}
static void merge_locations(VALUE *target, VALUE source) {
  if (NIL_P(source)) return;
  if (!RB_TYPE_P(source,T_ARRAY)) { add_unique(target,source); return; }
...
static bool has_location(VALUE list, VALUE item) {
  if (NIL_P(list)) return false;
...
static bool integer_p(VALUE value) {
  if (RB_INTEGER_TYPE_P(value)) return true;
  if (!RB_TYPE_P(value,T_FLOAT)) return false;
...
static VALUE cached_decimal(evaluator_t*e,VALUE value){if(RB_INTEGER_TYPE_P(value))return value;VALUE found=rb_hash_lookup2(e->decimals,value,Qundef);if(found!=Qundef)return found;found=decimal(value);rb_hash_aset(e->decimals,value,found);return found;}
static bool valid_number(evaluator_t*e,rule_t *r, VALUE value) {
  ...
  if (r->mask&NUM_MULTIPLE_OF) {
    ...
    return RTEST(rb_funcall(divisor,id_positive_p,0)) && RTEST(rb_funcall(rb_funcall(actual,id_remainder,1,divisor),id_zero_p,0));
  }
  return true;
}

static bool supported_value(VALUE value);
static bool json_equal(evaluator_t*e,VALUE left, VALUE right) {
  if(!supported_value(right)){e->unsupported_instance=true;return false;}
  ...
  if (RB_TYPE_P(left,T_ARRAY)) {
    ...
    for(long i=0;i<RARRAY_LEN(left);i++) if(!json_equal(e,rb_ary_entry(left,i),rb_ary_entry(right,i))) return false;
    return true;
  }
  if (RB_TYPE_P(left,T_HASH)) {
    if(RHASH_SIZE(left)!=RHASH_SIZE(right)) return false;
    VALUE keys=rb_funcall(left,id_keys,0);
    VALUE right_keys=rb_funcall(right,id_keys,0);for(long i=0;i<RARRAY_LEN(right_keys);i++)if(CLASS_OF(rb_ary_entry(right_keys,i))!=rb_cString){e->unsupported_instance=true;return false;}
    for(long i=0;i<RARRAY_LEN(keys);i++){VALUE k=rb_ary_entry(keys,i); if(!RTEST(rb_funcall(right,id_key_p,1,k))||!json_equal(e,rb_hash_aref(left,k),rb_hash_aref(right,k))) return false;}
    return true;
  }
...
static bool regexp_matches(evaluator_t*e,VALUE pattern,VALUE value){
  ...
  if(rb_enc_get_index(value)==rb_utf8_encindex()&&rb_enc_str_coderange(value)!=ENC_CODERANGE_BROKEN)return RTEST(rb_funcall(regexp,id_match_p,1,value));
  struct protected_call call={regexp,id_match_p,1,{value}};return protected_truth(&call);
}
static bool valid_string(evaluator_t *e, rule_t *r, VALUE value) {
  ...
  if(!NIL_P(r->as.string.pattern)&&!regexp_matches(e,r->as.string.pattern,value))return false;
...
static int boolean_program(VALUE program){program_t*p=PROGRAM_PTR(program);return p->length==1&&p->instructions[0].opcode==OP_BOOLEAN?(RTEST(p->instructions[0].operand)?1:0):-1;}
...
static bool active_enter(evaluator_t *e, VALUE source, VALUE instance) {
  for(size_t i=0;i<e->active_length;i++)if(e->active[i].source==source&&e->active[i].instance==instance)return false;
  if(e->active_length==e->active_capacity){e->active_capacity=e->active_capacity?e->active_capacity*2:8;REALLOC_N(e->active,active_entry_t,e->active_capacity);}
  e->active[e->active_length++]=(active_entry_t){source,instance};return true;
}
static void active_leave(evaluator_t *e,VALUE source,VALUE instance){e->active_length--;}
...
static VALUE recursive_target(evaluator_t*e,VALUE source,VALUE rule_obj){
  VALUE target=reference_target(e,source,rule_obj);rule_t*r=RULE_PTR(rule_obj);program_t*t=PROGRAM_PTR(target);
  if(!RB_TYPE_P(r->as.reference.fragment,T_STRING)||RSTRING_LEN(r->as.reference.fragment)||!t->recursive_anchor)return target;
  for(long i=0;i<RARRAY_LEN(e->dynamic_scope);i++){VALUE resource=rb_ary_entry(e->dynamic_scope,i);VALUE candidate=compiler_compile(e->compiler,rb_funcall(resource,id_root,0));program_t*p=PROGRAM_PTR(candidate);if(p->recursive_anchor)return candidate;}return target;
}
static VALUE dynamic_target(evaluator_t*e,VALUE source,VALUE rule_obj){
  VALUE target=reference_target(e,source,rule_obj);rule_t*r=RULE_PTR(rule_obj);program_t*t=PROGRAM_PTR(target);VALUE fragment=r->as.reference.fragment;
...
static VALUE cached_scope_target(evaluator_t*e,VALUE rule){long length=RARRAY_LEN(e->dynamic_scope);if(length>CACHED_SCOPE_LIMIT)return Qundef;for(size_t i=0;i<e->scope_cache_length;i++){scope_cache_entry_t*entry=&e->scope_cache[i];if(entry->rule!=rule||entry->length!=length)continue;long j=0;for(;j<length;j++)if(entry->resources[j]!=rb_ary_entry(e->dynamic_scope,j))break;if(j==length)return entry->target;}return Qundef;}
static void cache_scope_target(evaluator_t*e,VALUE rule,VALUE target){long length=RARRAY_LEN(e->dynamic_scope);if(length>CACHED_SCOPE_LIMIT)return;if(e->scope_cache_length==e->scope_cache_capacity){e->scope_cache_capacity=e->scope_cache_capacity?e->scope_cache_capacity*2:4;REALLOC_N(e->scope_cache,scope_cache_entry_t,e->scope_cache_capacity);}scope_cache_entry_t*entry=&e->scope_cache[e->scope_cache_length++];entry->rule=rule;entry->target=target;entry->length=(uint8_t)length;for(long i=0;i<length;i++)entry->resources[i]=rb_ary_entry(e->dynamic_scope,i);}
static VALUE safe_target(evaluator_t*e,VALUE source,VALUE rule,uint8_t opcode,int*state){struct target_call call={e,source,rule,opcode};VALUE cached=rb_hash_lookup2(e->resolved,rule,Qundef);if(cached==Qundef)return rb_protect(target_func,(VALUE)&call,state);if(opcode==OP_REF)return cached;VALUE target=cached_scope_target(e,rule);if(target!=Qundef)return target;target=target_func((VALUE)&call);cache_scope_target(e,rule,target);return target;}
...
static evaluation_t valid_array(evaluator_t*e,rule_t*r,VALUE value,evaluation_t prior,bool collect){
  ...
  if(r->as.array.unique)for(long i=1;i<len;i++)for(long j=0;j<i;j++)if(json_equal(e,rb_ary_entry(value,j),rb_ary_entry(value,i)))return evaluation(false);
  ...
  if(!NIL_P(r->as.array.contains)){long matches=0;VALUE matched=Qnil;for(long i=0;i<len;i++)if(evaluate_program(e,r->as.array.contains,rb_ary_entry(value,i)).valid){matches++;if(collect)add_unique(&matched,LONG2NUM(i));}if(matches<NUM2LONG(r->as.array.min_contains)||compare_values(LONG2NUM(matches),r->as.array.max_contains)>0)return evaluation(false);if(collect)merge_locations(&out.items,matched);}
  if(!NIL_P(r->as.array.unevaluated)){int boolean=boolean_program(r->as.array.unevaluated);for(long i=0;i<len;i++){VALUE index=LONG2NUM(i);if(has_location(prior.items,index)||has_location(out.items,index))continue;if(boolean==0)return evaluation(false);if(boolean<0&&!evaluate_program(e,r->as.array.unevaluated,rb_ary_entry(value,i)).valid)return evaluation(false);add_unique(&out.items,index);}}
  return out;
}

static bool required_present(VALUE object,VALUE names){if(NIL_P(names))return true;for(long i=0;i<RARRAY_LEN(names);i++)if(rb_hash_lookup2(object,rb_ary_entry(names,i),Qundef)==Qundef)return false;return true;}
typedef struct { evaluator_t*e; rule_t*r; evaluation_t*out; VALUE patterns; bool collect,valid; } object_properties_context_t;
static int valid_object_property(VALUE name,VALUE item,VALUE arg){
  ...
  if(CLASS_OF(name)!=rb_cString){c->e->unsupported_instance=true;c->valid=false;return ST_STOP;}
  bool matched=false;VALUE child=rb_hash_lookup2(c->r->as.object.properties,name,Qundef);
  if(child!=Qundef){matched=true;if(!evaluate_program(c->e,child,item).valid){c->valid=false;return ST_STOP;}if(c->collect)add_unique(&c->out->properties,name);}
  if(!NIL_P(c->patterns))for(long j=0;j<RARRAY_LEN(c->patterns);j++){VALUE pattern=rb_ary_entry(c->patterns,j);if(regexp_matches(c->e,pattern,name)){matched=true;if(!evaluate_program(c->e,rb_hash_aref(c->r->as.object.patterns,pattern),item).valid){c->valid=false;return ST_STOP;}if(c->collect)add_unique(&c->out->properties,name);}}
  if(!matched&&!NIL_P(c->r->as.object.additional)){if(!evaluate_program(c->e,c->r->as.object.additional,item).valid){c->valid=false;return ST_STOP;}if(c->collect)add_unique(&c->out->properties,name);}
  return ST_CONTINUE;
}
typedef struct { evaluator_t*e; VALUE program; bool valid; } property_name_context_t;
static int valid_property_name(VALUE name,VALUE item,VALUE arg){property_name_context_t*c=(property_name_context_t*)arg;if(CLASS_OF(name)!=rb_cString){c->e->unsupported_instance=true;c->valid=false;return ST_STOP;}if(!evaluate_program(c->e,c->program,name).valid){c->valid=false;return ST_STOP;}return ST_CONTINUE;}
typedef struct { evaluator_t*e; VALUE program; evaluation_t prior,*out; int boolean; bool valid; } unevaluated_property_context_t;
static int valid_unevaluated_property(VALUE name,VALUE item,VALUE arg){unevaluated_property_context_t*c=(unevaluated_property_context_t*)arg;if(CLASS_OF(name)!=rb_cString){c->e->unsupported_instance=true;c->valid=false;return ST_STOP;}if(has_location(c->prior.properties,name)||has_location(c->out->properties,name))return ST_CONTINUE;if(c->boolean==0){c->valid=false;return ST_STOP;}if(c->boolean<0&&!evaluate_program(c->e,c->program,item).valid){c->valid=false;return ST_STOP;}add_unique(&c->out->properties,name);return ST_CONTINUE;}
static evaluation_t valid_object(evaluator_t*e,rule_t*r,VALUE value,evaluation_t prior,bool collect){
  ...
  if(!RHASH_EMPTY_P(r->as.object.properties)||!NIL_P(r->as.object.patterns)||!NIL_P(r->as.object.additional)){object_properties_context_t properties={e,r,&out,r->as.object.pattern_names,collect,true};rb_hash_foreach(value,valid_object_property,(VALUE)&properties);if(!properties.valid)return evaluation(false);}
  if(!NIL_P(r->as.object.property_names)){property_name_context_t names={e,r->as.object.property_names,true};rb_hash_foreach(value,valid_property_name,(VALUE)&names);if(!names.valid)return evaluation(false);}
  VALUE dep_sets[]={r->as.object.dependencies,r->as.object.dependent_required};for(int d=0;d<2;d++)if(!NIL_P(dep_sets[d])){VALUE names=rb_funcall(dep_sets[d],id_keys,0);for(long i=0;i<RARRAY_LEN(names);i++){VALUE name=rb_ary_entry(names,i);if(rb_hash_lookup2(value,name,Qundef)==Qundef)continue;VALUE dep=rb_hash_aref(dep_sets[d],name);if(RB_TYPE_P(dep,T_ARRAY)){if(!required_present(value,dep))return evaluation(false);}else{evaluation_t x=evaluate_program_mode(e,dep,value,collect);if(!x.valid)return evaluation(false);if(collect)merge_locations(&out.properties,x.properties);}}}
  if(!NIL_P(r->as.object.dependent_schemas)){VALUE names=rb_funcall(r->as.object.dependent_schemas,id_keys,0);for(long i=0;i<RARRAY_LEN(names);i++){VALUE name=rb_ary_entry(names,i);if(rb_hash_lookup2(value,name,Qundef)==Qundef)continue;evaluation_t x=evaluate_program_mode(e,rb_hash_aref(r->as.object.dependent_schemas,name),value,collect);if(!x.valid)return evaluation(false);if(collect)merge_locations(&out.properties,x.properties);}}
  if(!NIL_P(r->as.object.unevaluated)){unevaluated_property_context_t unevaluated={e,r->as.object.unevaluated,prior,&out,boolean_program(r->as.object.unevaluated),true};rb_hash_foreach(value,valid_unevaluated_property,(VALUE)&unevaluated);if(!unevaluated.valid)return evaluation(false);}
  return out;
}

static bool supported_value(VALUE value){
  if(NIL_P(value)||value==Qtrue||value==Qfalse||RB_INTEGER_TYPE_P(value))return true;
  if(CLASS_OF(value)==rb_cFloat)return isfinite(RFLOAT_VALUE(value));
  return CLASS_OF(value)==rb_cString||CLASS_OF(value)==rb_cArray||CLASS_OF(value)==rb_cHash;
}

static evaluation_t evaluate_program_mode(evaluator_t*e,VALUE program,VALUE instance,bool collect){
  program_t*p=PROGRAM_PTR(program);
  if(p->length==0)return evaluation(true);
  if(p->length==1&&p->instructions[0].opcode==OP_BOOLEAN)return evaluation(RTEST(p->instructions[0].operand));
  if(p->length==1)switch(p->instructions[0].opcode){
    case OP_TYPE_NULL:return evaluation(NIL_P(instance));case OP_TYPE_BOOLEAN:return evaluation(instance==Qtrue||instance==Qfalse);case OP_TYPE_OBJECT:return evaluation(RB_TYPE_P(instance,T_HASH));case OP_TYPE_ARRAY:return evaluation(RB_TYPE_P(instance,T_ARRAY));case OP_TYPE_STRING:return evaluation(RB_TYPE_P(instance,T_STRING));default:break;
  }
  if(p->length==1){uint8_t opcode=p->instructions[0].opcode;if((opcode==OP_STRING&&!RB_TYPE_P(instance,T_STRING))||(opcode==OP_ARRAY&&!RB_TYPE_P(instance,T_ARRAY))||(opcode==OP_OBJECT&&!RB_TYPE_P(instance,T_HASH)))return evaluation(true);}
  if(!supported_value(instance)){e->unsupported_instance=true;return evaluation(false);}
  if(p->length==1){instruction_t*ins=&p->instructions[0];rule_t*r;switch(ins->opcode){
    case OP_TYPE_NULL:return evaluation(NIL_P(instance));case OP_TYPE_BOOLEAN:return evaluation(instance==Qtrue||instance==Qfalse);case OP_TYPE_OBJECT:return evaluation(RB_TYPE_P(instance,T_HASH));case OP_TYPE_ARRAY:return evaluation(RB_TYPE_P(instance,T_ARRAY));case OP_TYPE_NUMBER:return evaluation(number_p(instance));case OP_TYPE_INTEGER:return evaluation(integer_p(instance));case OP_TYPE_STRING:return evaluation(RB_TYPE_P(instance,T_STRING));
    case OP_TYPES:r=RULE_PTR(ins->operand);return evaluation((r->mask&instance_type(instance,(r->mask&TYPE_INTEGER)!=0))!=0);
    case OP_ENUM:for(long j=0;j<RARRAY_LEN(ins->operand);j++)if(json_equal(e,rb_ary_entry(ins->operand,j),instance))return evaluation(true);return evaluation(false);
    case OP_CONST:return evaluation(json_equal(e,ins->operand,instance));
    case OP_NUMBER:r=RULE_PTR(ins->operand);return evaluation(!number_p(instance)||valid_number(e,r,instance));
    case OP_STRING:r=RULE_PTR(ins->operand);return evaluation(!RB_TYPE_P(instance,T_STRING)||valid_string(e,r,instance));
    case OP_TYPED_NUMBER:r=RULE_PTR(ins->operand);return evaluation(number_p(instance)&&valid_number(e,r,instance));
    case OP_TYPED_INTEGER:r=RULE_PTR(ins->operand);return evaluation(integer_p(instance)&&valid_number(e,r,instance));
    case OP_TYPED_STRING:r=RULE_PTR(ins->operand);return evaluation(RB_TYPE_P(instance,T_STRING)&&valid_string(e,r,instance));
    default:break;
  }}
  evaluation_t out=evaluation(true);bool entered=false;
  collect=collect||(p->flags&FLAG_EVALUATION);
  if(p->flags&FLAG_DYNAMIC_SCOPE){VALUE resource=p->resource;if(RARRAY_LEN(e->dynamic_scope)==0||rb_ary_entry(e->dynamic_scope,-1)!=resource){rb_ary_push(e->dynamic_scope,resource);entered=true;}}
  for(size_t i=0;i<p->length&&out.valid&&!e->unsupported_instance;i++){instruction_t*ins=&p->instructions[i];rule_t*r=NULL;if(ins->opcode==OP_TYPES||ins->opcode>=OP_CONDITIONAL)r=RULE_PTR(ins->operand);evaluation_t x=evaluation(true);
    switch(ins->opcode){
      ...
      case OP_ENUM:out.valid=false;for(long j=0;j<RARRAY_LEN(ins->operand);j++)if(json_equal(e,rb_ary_entry(ins->operand,j),instance)){out.valid=true;break;}break;
      case OP_CONST:out.valid=json_equal(e,ins->operand,instance);break;
      ...
      case OP_NUMBER:if(number_p(instance))out.valid=valid_number(e,r,instance);break;case OP_STRING:if(RB_TYPE_P(instance,T_STRING))out.valid=valid_string(e,r,instance);break;
      ...
      case OP_TYPED_NUMBER:out.valid=number_p(instance)&&valid_number(e,r,instance);break;case OP_TYPED_INTEGER:out.valid=integer_p(instance)&&valid_number(e,r,instance);break;case OP_TYPED_STRING:out.valid=RB_TYPE_P(instance,T_STRING)&&valid_string(e,r,instance);break;
...
static VALUE compiler_evaluator(int argc,VALUE*argv,VALUE self){
  ...
  RB_OBJ_WRITE(evaluator,&e->regexps,rb_hash_new());RB_OBJ_WRITE(evaluator,&e->resolved,rb_hash_new());rb_funcall(e->resolved,id_compare_by_identity,0);RB_OBJ_WRITE(evaluator,&e->decimals,rb_hash_new());
  RB_OBJ_WRITE(evaluator,&e->dynamic_scope,rb_ary_new());
  e->content=RTEST(rb_hash_aref(options,sym_content));e->format=RTEST(rb_hash_aref(options,sym_format));e->unsupported_instance=false;return evaluator;
}
...
static VALUE evaluator_valid(VALUE self,VALUE instance){evaluator_t*e;TypedData_Get_Struct(self,evaluator_t,&evaluator_type,e);if(PROGRAM_PTR(e->root)->flags&FLAG_DYNAMIC_SCOPE)rb_ary_clear(e->dynamic_scope);e->active_length=0;e->unsupported_instance=false;bool valid=evaluate_program(e,e->root,instance).valid;if(e->unsupported_instance)return rb_funcall(ruby_evaluator(e),id_valid_p,1,instance);return valid?Qtrue:Qfalse;}
...
static evaluation_t evaluate_detail(evaluator_t*e,VALUE program,VALUE instance){if(!supported_value(instance)){e->unsupported_instance=true;return evaluation(false);}program_t*p;TypedData_Get_Struct(program,program_t,&program_type,p);long before=e->error_count;evaluation_t out=evaluation(true);bool entered=false;if(p->flags&FLAG_DYNAMIC_SCOPE){VALUE resource=p->resource;if(RARRAY_LEN(e->dynamic_scope)==0||rb_ary_entry(e->dynamic_scope,-1)!=resource){rb_ary_push(e->dynamic_scope,resource);entered=true;}}
  for(size_t i=0;i<p->length&&!e->unsupported_instance;i++){instruction_t*ins=&p->instructions[i];rule_t*r=NULL;if(rb_typeddata_is_kind_of(ins->operand,&rule_type))TypedData_Get_Struct(ins->operand,rule_t,&rule_type,r);evaluation_t x=evaluation(true);switch(ins->opcode){case OP_BOOLEAN:if(!RTEST(ins->operand))add_error(e,STATIC_STRING(FALSE_SCHEMA),error_message(id_error_false_schema,0,NULL),false);break;case OP_REF:case OP_RECURSIVE_REF:case OP_DYNAMIC_REF:{int state=0;VALUE target=safe_target(e,program,ins->operand,ins->opcode,&state);VALUE keyword=ins->opcode==OP_REF?STATIC_STRING(REF):ins->opcode==OP_RECURSIVE_REF?STATIC_STRING(RECURSIVE_REF):STATIC_STRING(DYNAMIC_REF);if(state){VALUE error=rb_errinfo();if(!RTEST(rb_obj_is_kind_of(error,eResolutionError)))rb_jump_tag(state);VALUE message=rb_funcall(error,id_message,0);rb_set_errinfo(Qnil);add_error(e,keyword,message,false);}else{x=detail_reference(e,program,target,instance,keyword);merge_evaluation(&out,x);}break;}
    ...
    case OP_ENUM:{bool ok=false;for(long j=0;j<RARRAY_LEN(ins->operand);j++)if(json_equal(e,rb_ary_entry(ins->operand,j),instance)){ok=true;break;}if(!ok)add_message0(e,STATIC_STRING(ENUM),id_error_enum);break;}case OP_CONST:if(!json_equal(e,ins->operand,instance))add_message0(e,STATIC_STRING(CONST),id_error_const);break;
  ...
  if(entered)rb_ary_pop(e->dynamic_scope);
  out.valid=e->error_count==before;return out;}
static VALUE evaluator_validate(VALUE self,VALUE instance){evaluator_t*e;TypedData_Get_Struct(self,evaluator_t,&evaluator_type,e);RB_OBJ_WRITE(self,&e->errors,rb_ary_new());RB_OBJ_WRITE(self,&e->instance_path,rb_ary_new());RB_OBJ_WRITE(self,&e->schema_path,rb_ary_new());if(PROGRAM_PTR(e->root)->flags&FLAG_DYNAMIC_SCOPE)rb_ary_clear(e->dynamic_scope);e->active_length=0;e->error_count=0;e->unsupported_instance=false;evaluate_detail(e,e->root,instance);if(e->unsupported_instance){RB_OBJ_WRITE(self,&e->errors,Qnil);return rb_funcall(ruby_evaluator(e),id_validate,1,instance);}VALUE result=rb_class_new_instance(1,&e->errors,cResult);RB_OBJ_WRITE(self,&e->errors,Qnil);return result;}
