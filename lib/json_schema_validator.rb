    ...
    class Evaluator
      ...
      private def valid_string?(node, value)
        ...
        if format_asserted?(node) && schema.key?("format")
          return false unless valid_format?(schema["format"], value)
        end
      ...
      private def check_string(node, value, path, schema_path)
        ...
        if format_asserted?(node) && schema.key?("format")
          ...
          add_error("format", path, append(schema_path, "format"), "string is not a valid #{format}") unless valid_format?(format, value)
        end
      ...
      private def valid_format?(format, value)
        case format
